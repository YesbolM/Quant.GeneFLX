...
cfm1fWS<-cfm1f[,c(1,7,13:14)]
str(cfm1fWS)
#Make round.sec into integers by taking factor codes
cfm1fWS$round.sec <- as.numeric(cfm1fWS$round.sec)
#Drop unused levels
cfm1fWS<-droplevels(cfm1fWS) 
str(cfm1fWS)
#round: 4 levels
length(unique(cfm1fWS$round.sec))
#round.sec: 27 levels

#Change ids into numeric values
cfm1fWS$animal<-gsub("M","1",cfm1fWS$animal)
cfm1fWS$animal<-gsub("F","2",cfm1fWS$animal)
cfm1fWS$animal<-gsub("m","3",cfm1fWS$animal)
cfm1fWS$animal<-gsub("f","4",cfm1fWS$animal)
cfm1fWS$animal<-gsub("-","",cfm1fWS$animal)

#Change name of round.sec to NSEC
names(cfm1fWS)[4]<-"NSEC"

#write data file
setwd("E:\\Lund\\Flies\\Yesbol\\Quant gen manuscript\\Wombat analysis\\cfm1fWS")
write.table(cfm1fWS, file="cfm1fWS_dat.dat", row.names = FALSE, quote = FALSE, col.names = FALSE)
...
#write pedigree file
setwd("E:\\Lund\\Flies\\Yesbol\\Quant gen manuscript\\Wombat analysis\\cfm1fWS")
write.table(ped.cfm1, file="cfm1fWS_ped.ped", row.names = FALSE, quote = FALSE, col.names = FALSE)

#Get starting values for par file from MCMCglmm results
setwd("E:\\Lund\\Flies\\Yesbol\\Quant gen manuscript\\univariate models")
rep1CFMfWS <- readRDS("rep1CFM.fWS")
library(MCMCglmm)
posterior.mode(rep1CFMfWS$VCV)
#animal    round.sec        units 
#0.0020768812 0.0003171822 0.0007289045 

# Testing significance of random effects
#logL for model with animal: 1099.171
#logL for model without animal: 1083.553
q <- 2*(1099.171-1083.553)
q
#q=31.236
